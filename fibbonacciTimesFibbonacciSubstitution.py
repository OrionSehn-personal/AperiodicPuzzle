...
class Tile:
    ...
    def substitute(self):
        ...
        if (self.subtype == 0):
            scalar1 = PHI/(1+PHI)
            scalar2 = 1 /(1+PHI)
            leftvector = ((self.point2[0] - self.point1[0]),
             (self.point2[1] - self.point1[1]))
            midvector = ((self.point1[0] - self.point3[0]),
             (self.point1[1] - self.point3[1]))
            rightvector = ((self.point4[0] - self.point1[0]),
             (self.point4[1] - self.point1[1]))
            rightpoint = (self.point1[0]+(rightvector[0]*scalar1),
             self.point1[1] + (rightvector[1]*scalar1))
            leftpoint = ((self.point1[0]+(leftvector[0]*scalar1),
             self.point1[1] + (leftvector[1]*scalar1)))

            #first tile
            tile1 = Tile(self.point1, leftpoint, self.point3,
             rightpoint, 1)
            newtiles.append(tile1)
            #second tile
            botleftpoint = ((self.point2[0]+midvector[0]),
             (self.point2[1]+ midvector[1]))
            tile2 = Tile(self.point2, self.point3,
             leftpoint, botleftpoint, 0)
            newtiles.append(tile2)            
            #third tile
            botrightpoint = ((self.point4[0]+midvector[0]),
             (self.point4[1]+ midvector[1]))
            tile3 = Tile(self.point4, botrightpoint,
             rightpoint, self.point3, 0)
        ...
        if (self.subtype == 1):
            ...
            leftvector = ((self.point2[0] - self.point1[0]),
             (self.point2[1] - self.point1[1]))
            midvector = ((self.point3[0] - self.point1[0]),
             (self.point3[1] - self.point1[1]))
            rightvector = ((self.point4[0] - self.point1[0]),
             (self.point4[1] - self.point1[1]))
            midpoint = (self.point1[0]+(midvector[0]*scalar1),
             self.point1[1] + (midvector[1]*scalar1))
            rightpoint = (self.point1[0]+(rightvector[0]*scalar2),
             self.point1[1] + (rightvector[1]*scalar2))
            leftpoint = ((self.point1[0]+(leftvector[0]*scalar2),
            ...
            #third tile
            toprightvector = ((self.point3[0] - self.point4[0]),
             (self.point3[1] - self.point4[1]))
            botleftpoint = ((self.point1[0]+toprightvector[0]),
             (self.point1[1]+ toprightvector[1]))
            tile3 = Tile(self.point1, botleftpoint,
             leftpoint, midpoint, 0)
            newtiles.append(tile3)

            #fourth tile
            topleftvector = ((self.point3[0] - self.point2[0]),
             (self.point3[1] - self.point2[1]))
            botrightpoint = ((self.point1[0]+topleftvector[0]),
             (self.point1[1]+ topleftvector[1]))
            tile4 = Tile(self.point1, midpoint, 
            rightpoint, botrightpoint, 0)
